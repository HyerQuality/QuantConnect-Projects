    ...
    def Update(self, algorithm, data):
        ...
        for symbol, symbolData in self.SPXL.items():
            ...
            if self.ReadyCheck(symbol, symbolData):
                ...
                elif VixHandler.PreviousVixClose.Current.Value <= VixHandler.DeviationVix[0] and \
                        symbolData.RSIDeviations[3] < symbolData.RSI.Current.Value < symbolData.RSIDeviations[5] and not \
                algorithm.Portfolio[symbol].Invested:
                    insights.append(
                        Insight.Price(symbol, timedelta(days=2), InsightDirection.Up, float(0.0075), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[SPXL]: The previous day VIX close is less than or equal to 3 standard deviations below its 1-year mean and SPXL RSI is showing upward momentum but not overbought conditions and we are not currently invested in SPXL")
                ...
                # If the overnight gap is between 1 and 2 STD from the mean
                    ...
                    insights.append(
                        Insight.Price(symbol, timedelta(hours=1), InsightDirection.Up, float(0.001), None, None, 1.0))
                    symbolData.Trade = False
                ...
                elif (int(4) < VixHandler.FiveDayVixPercentMoveSTD < int(
                        5)) and VixHandler.PreviousVixClose.Current.Value < VixHandler.DeviationVix[2]:
                    insights.append(Insight.Price(symbol, OneDay, InsightDirection.Up, float(0.0075), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[SPXL]: The percent change of the VIX over the last 5 trading days has been relatively volatile, but the spot VIX is under 1.5 STD from the mean")

                # If the 5-day average of VIX closes is between -10% and -7%
                elif -int(10) <= VixHandler.SixDayVixAverage < -int(7) and symbolData.Trade:
                    insights.append(
                        Insight.Price(symbol, timedelta(days=1), InsightDirection.Up, float(0.0075), None, None, 1.0))
                    symbolData.Trade = False
        ...
        if self.TradingWindow:
            for symbol, symbolData in self.SPXL.items():
                ...
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.05):
                    RiskInsights.append(
                        Insight.Price(symbol, RemainingTradingDay, InsightDirection.Flat, float(0.01), None, None, 0.5))
                    symbolData.Trade = False
        ...
        for symbol, symbolData in self.SPXS.items():
            ...
            if self.ReadyCheck(symbol, symbolData):
                ...
                if symbolData.GapSignal[2] > Global.OpenClose[symbol][2] > symbolData.GapSignal[1]:
                    insights.append(
                        Insight.Price(SPXL, timedelta(days=1), InsightDirection.Up, float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[SPXS]: Gapped up a reasonable amount, buy the inverse SPXL as a contrarian play - essentially buying the dip")

                # If SPXS is extremely oversold
                elif (symbolData.Mean - 3 * symbolData.STD) < symbolData.EMACross.Current.Value < (
                        symbolData.Mean - 2 * symbolData.STD):
                    insights.append(
                        Insight.Price(symbol, timedelta(hours=3), InsightDirection.Up, float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                ...
                elif VixHandler.FiveDayVixPercentMoveSTD < int(2) and symbolData.EMACross.Current.Value > (
                        symbolData.Mean - 0 * symbolData.STD):
                    insights.append(
                        Insight.Price(symbol, timedelta(days=3), InsightDirection.Up, float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[SPXS]: The standard deviation of the 5-day VIX percent change is extremely low and SPXS price momentum is to the upside")
                ...
                # If RSI is elevated and the previous 2 RSI points are rising, SPXS is trending upward, the standard deviation of the 5-day VIX percent change is normal, and there is not currently an open SPXL position
                    ...
                    insights.append(
                        Insight.Price(SPXL, timedelta(days=4), InsightDirection.Up, float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[SPXS]: RSI is elevated and the previous 2 RSI points are rising, SPXS is trending upward, the standard deviation of the 5-day VIX percent change is normal, and there is not currently an open SPXL position [ Current SPXS RSI: {0} ]".format(
        ...
        if self.TradingWindow:
            for symbol, symbolData in self.SPXS.items():
                ...
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.04):
                    RiskInsights.append(
                        Insight.Price(UVXY, RemainingTradingDay, InsightDirection.Down, float(0.01), None, None, 0.5))
                    symbolData.Trade = False
