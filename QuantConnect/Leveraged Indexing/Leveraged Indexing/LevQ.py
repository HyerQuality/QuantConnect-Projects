        ...
        for symbol, symbolData in self.TQQQ.items():
            ...
            if self.ReadyCheck(algorithm, symbol, symbolData):
                ...
                if symbolData.RSIDeviations[0] <= symbolData.RSI.Current.Value <= symbolData.RSIDeviations[1]:
                    ...
                    algorithm.Log("[TQQQ]: RSI value is between two and three standard deviations from the mean [ Current TQQQ RSI: {0} ]".format(symbolData.RSI.Current.Value))
                ...
                elif ((symbolData.Mean - 1.5*symbolData.STD) <= symbolData.EMACross.Current.Value <= (symbolData.Mean - 1*symbolData.STD)) and not algorithm.Portfolio[symbol].Invested and VixHandler.FiveDayVixPercentMoveSTD <= int(7):
                    insights.append(Insight.Price(symbol, Expiry.EndOfDay, InsightDirection.Up, float(0.01), None, None, 1.0))
                    Global.TradeTriggers[symbol] = False
                    algorithm.Log("[TQQQ]: We are not currently invested in TQQQ, the EMA Cross is between -1.5 and -1 standard deviations from the mean, and the 5 day standard deviation of VIX close is less than 7")

                # If the average 5-day VIX closing % is bewteen -10% and -5% and the standard deviation of those 5 closes is not very small
                elif -int(10) <= round(VixHandler.SixDayVixAverage,2) <= -int(5) and round(VixHandler.FiveDayVixPercentMoveSTD,2) >= int(2):
                    insights.append(Insight.Price(symbol, timedelta(hours=6), InsightDirection.Up, float(0.01), None, None, 1.0))
                    Global.TradeTriggers[symbol] = False
                    algorithm.Log("[TQQQ]: The average 5-day VIX closing % is bewteen -10% and -5% and the standard deviation of those 5 closes is not very small")
    ...
    def ManagePosition(self, algorithm):
        ...
        for symbol, symbolData in self.TQQQ.items():
            if Global.MarketIsOpen and not algorithm.Portfolio[SPXS].Invested:
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.045):
                    RiskInsights.append(Insight.Price(SPXS, timedelta(hours=1.5), InsightDirection.Up, float(0.01), None, None, 0.5))
        ...
        for symbol, symbolData in self.SQQQ.items():
            ...
            if self.ReadyCheck(algorithm, symbol, symbolData):
                ...
                if symbolData.RSIDeviations[0] <= symbolData.RSI.Current.Value <= symbolData.RSIDeviations[1]:
                    insights.append(Insight.Price(symbol, timedelta(days=2), InsightDirection.Up, float(0.01), None, None, 1.0))
                    Global.TradeTriggers[symbol] = False
                    algorithm.Log("[SQQQ]: RSI value is between two and three standard deviations below the mean [ Currrent SQQQ RSI : {0} ]".format(symbolData.RSI.Current.Value))
                ...
                # If SQQQ split and we aren't already invested in TQQQ
                    ...
                    insights.append(Insight.Price(TQQQ, timedelta(days=10), InsightDirection.Up, float(0.01), None, None, 0.5))
                    insights.append(Insight.Price(symbol, timedelta(days=2), InsightDirection.Up, float(0.01), None, None, 0.5))
                ...
                # If the rate of change of the current RSI value to the previous RSI value is 150% or greater
                    ...
                    insights.append(Insight.Price(TQQQ, timedelta(days=2), InsightDirection.Up, float(0.01), None, None, 1.0))
                    Global.TradeTriggers[symbol] = False
                    algorithm.Log("[SQQQ]: Rate of change of the current RSI value to the previous RSI value is 150% or greater [ Currrent SQQQ RSI : {0} ]".format(symbolData.RSI.Current.Value))
                ...
                # If the current RSI value is between 2 and 3 standard deviations from the mean
                    ...
                    insights.append(Insight.Price(TQQQ, timedelta(days=2.5), InsightDirection.Up, float(0.01), None, None, 1.0))
                    Global.TradeTriggers[symbol] = False
                    algorithm.Log("[SQQQ]: Current RSI value is between 2 and 3 standard deviations from the mean [ Currrent SQQQ RSI : {0} ]".format(symbolData.RSI.Current.Value))
        ...
        for symbol, symbolData in self.SQQQ.items():
            if (
                ...
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.05):
                    RiskInsights.append(Insight.Price(TQQQ, timedelta(hours=0.5), InsightDirection.Up, float(0.01), None, None, 0.5))
