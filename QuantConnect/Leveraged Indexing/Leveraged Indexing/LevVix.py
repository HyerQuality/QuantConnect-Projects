...
##-----------------Update-------------------------------------------------------##
    ...
    def Update(self, algorithm, data):
        ...
        if datetime.combine( algorithm.Time.today(), time(16,00) ) - datetime.combine(algorithm.Time.today(), algorithm.Time.time() ) > timedelta(minutes=10):
        ...
        for symbol, symbolData in self.LongVIX.items():
            ...
            if self.ReadyCheck(symbol, symbolData, algorithm):
                ...
                # If the standard deviation of the 5 day rolling percent change of the VIX spot price is near zero and RSI is not extended in either direction
                    ...
                    and algorithm.Securities[symbol].Price < ((Global.OpenClose[symbol][0] + Global.OpenClose[symbol][1])/2) * 1.025
                    ...
                    and symbolData.RSIDeviations[7] >= symbolData.RSI.Current.Value >= symbolData.RSIDeviations[0]
                        ...
                        # If the current portfolio drawdown is less than 7%
                        # if Global.PortfolioDrawdown > -float(0.07):
                        insights.append(Insight.Price(symbol, HoldingPeriod, InsightDirection.Up, float(0.05), None, None, 1.0))
                        symbolData.Trade = False
                        ...
                        # else:
                        #     insights.append(Insight.Price(symbol, HoldingPeriod, InsightDirection.Up, float(0.05), None, None, 0.5))
                        #     insights.append(Insight.Price(TQQQ, Expiry.EndOfDay, InsightDirection.Up, float(0.05), None, None, 0.5))
                        #     symbolData.Trade = False
                        #     algorithm.Log("[LONG UVXY]: The standard deviation of the 5-day rolling percent change of the VIX spot price is near zero, RSI is not extended in either direction and portfolio drawdown is below -7.0% [ Current UVXY RSI: {0} ]".format(symbolData.RSI.Current.Value))
                ...
                # If the moving average cross over is between 4 and 5 standard deviations below its long term average value
                    ...
                    insights.append(Insight.Price(symbol, HoldingPeriod, InsightDirection.Up, float(0.01), None, None, 1.0))

                    if not algorithm.Portfolio[symbol].Invested:
                        algorithm.Log("[LONG UVXY]: The moving average cross over is between 4 and 5 standard deviations below its long term average value. Will continue emitting insights over this duration to ensure a holding period from trend reversal.")
...
##-----------------Create Alpha specific risk management---------------------------------##
    ...
    def ManagePosition(self, algorithm):
        ...
        for symbol, symbolData in self.LongVIX.items():
            ...
            if self.ReadyCheck and algorithm.Portfolio[symbol].IsLong:

                # As the long vol position approaches a 5% in an environment where the closing VIX has been very consisntent for the past 5 days shift some funds into long indexes to hedge.
                if (
                    algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.045)
                    # and not algorithm.Portfolio[SPXL].Invested
                    # and not algorithm.Portfolio[TQQQ].Invested
                    and int(0) < VixHandler.FiveDayVixPercentMoveSTD < int(2)
                    ):

                    if not algorithm.Portfolio[SPXL].Invested:
                        RiskInsights.append(Insight.Price(SPXL, timedelta(days=5), InsightDirection.Up, float(0.01), None, None, 0.5))
                        algorithm.Log("[Long UVXY]: Position is approaching a 5% loss. Hedging into long SPXL")

                    if not algorithm.Portfolio[TQQQ].Invested:
                        RiskInsights.append(Insight.Price(TQQQ, timedelta(days=5), InsightDirection.Up, float(0.01), None, None, 0.5))
                        algorithm.Log("[Long UVXY]: Position is approaching a 5% loss. Hedging into long TQQQ")
...
##-----------------Generate insights---------------------------------------------------##
        ...
        for symbol, symbolData in self.ShortVIX.items():
            ...
            # Check share availabilty
            if Global.NoSharesAvailable:
                insights.append(Insight.Price(symbol, RemainingTradingDay, InsightDirection.Flat))
                symbolData.Trade = False
            ...
            if self.ReadyCheck(symbol, symbolData, algorithm):
                ...
                if (( (symbolData.Mean - 1*symbolData.STD) < symbolData.EMACross.Current.Value < (symbolData.Mean - 0*symbolData.STD) )
                    ...
                    insights.append(Insight.Price(symbol, timedelta(days=3), InsightDirection.Down, -float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log("[SHORT UVXY]: EMACross is negative but not extended, RSI is extended to the upside, UVXY gapped down, but less than 1 STD from the mean gap down, the VIX index isn't fluctuating wildly and there are currently no short UVXY positions")
                ...
                # If price is trending down, the current price is 3.5% below the average of the open and previous close, RSI is above half the average, and no other assets are being held
                    ...
                    and not algorithm.Portfolio[symbol].Invested
                        ...
                        insights.append(Insight.Price(symbol, RemainingTradingDay, InsightDirection.Down, -float(0.01), None, None, 1.0))
                        symbolData.Trade = False
                        ...
                        #     HoldingPeriod = max(datetime.combine(algorithm.Time.today(), time(15,58)) - datetime.combine(algorithm.Time.today(), algorithm.Time.time()), timedelta(minutes=10))

                        #     insights.append(Insight.Price(symbol, HoldingPeriod, InsightDirection.Down, -float(0.01), None, None, 0.33))
                        #     insights.append(Insight.Price(SQQQ, HoldingPeriod, InsightDirection.Up, float(0.01), None, None, 0.33))
                        #     insights.append(Insight.Price(SPXL, HoldingPeriod, InsightDirection.Up, float(0.01), None, None, 0.33))
...
##-----------------Create Alpha specific risk management---------------------------------##
    ...
    def ManagePosition(self, algorithm, symbol, symbolData):
        ...
        if (
            ...
            if symbolData.PriceWindow[0]/symbolData.PriceWindow[1] > 1.03 and time(9,33) <= algorithm.Time.time():

                if datetime.combine( algorithm.Time.today(), time(16,00) ) - datetime.combine(algorithm.Time.today(), algorithm.Time.time() ) > timedelta(minutes=60):
                    RemainingTradingDay = Expiry.EndOfDay
                else:
                    RemainingTradingDay = timedelta(minutes=60)

                RiskInsights.append(Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, float(0.01), None, None, 1.0))
                symbolData.Trade = False
                algorithm.Log("[SHORT UVXY]: Massive gap up in UVXY. Closing short and going long")
...
##-----------------Checks that certain criteria are met before trading-------------------##
    def ReadyCheck(self, symbol, symbolData, algorithm):
        ...
        and not self.OpenOrders
        # and algorithm.Shortable(symbol, 1)
        ):
