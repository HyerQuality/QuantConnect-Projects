...
class LongVol(AlphaModel):
    ...
    def Update(self, algorithm, data):
        ...
        if datetime.combine(algorithm.Time.today(), time(16, 00)) - datetime.combine(algorithm.Time.today(),
                                                                                     algorithm.Time.time()) > timedelta(
                minutes=10):
            RemainingTradingDay = Expiry.EndOfDay
        else:
            RemainingTradingDay = timedelta(minutes=10)
        ...
        for symbol, symbolData in self.LongVIX.items():
            ...
            if data.ContainsKey(symbol):
                symbolData.PriceWindow.Add(algorithm.Securities[symbol].Close)

            # Reset the trade signal each day
            ...
            if self.ReadyCheck(symbol, symbolData, algorithm):
                ...
                if (VixHandler.DeviationVixChange[6] <= VixHandler.vixPercentMoveList[-1] <
                        VixHandler.DeviationVixChange[7]):
                    ...
                    if (not algorithm.Portfolio[symbol].Invested
                        ...
                        insights.append(
                            Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, float(0.05), None, None,
                                          1.0))
                        symbolData.Trade = False
                        algorithm.Log(
                            "[LONG UVXY]: The previous day VIX % move was between +3 and +4 standard deviations from the mean and we are not currently invested and UVXY didn't gap up more than 10%")
                ...
                # If UVXY gapped down more than 4 standard deviations from its average gap down
                    ...
                    insights.append(
                        Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, float(0.05), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[LONG UVXY]: UVXY gapped down more than 4 standard deviations from its average gap down")
                ...
                # If the standard deviation of the 5 day rolling percent change of the VIX spot price is near zero and RSI is not extended in either direction
                    ...
                    HoldingPeriod = timedelta(hours=10)

                    # If the current portfolio drawdown is less than 12.5%
                    if Global.PortfolioDrawdown > -float(0.125):
                        insights.append(
                            Insight.Price(symbol, HoldingPeriod, InsightDirection.Up, float(0.05), None, None, 1.0))
                        symbolData.Trade = False
                        algorithm.Log(
                            "[LONG UVXY]: The standard deviation of the 5-day rolling percent change of the VIX spot price is near zero, RSI is not extended in either direction and portfolio drawdown is above -12.5% [ Current UVXY RSI: {0} ]".format(
                    ...
                    # Take a long-short position if the portfolio drawdown is greater than or equal to 12.5% because the market is likely experiencing very frequent high magnitude swings
                        ...
                        insights.append(
                            Insight.Price(symbol, HoldingPeriod, InsightDirection.Up, float(0.05), None, None, 0.5))
                        insights.append(
                            Insight.Price(TQQQ, HoldingPeriod, InsightDirection.Up, float(0.05), None, None, 0.5))
                        symbolData.Trade = False
                        algorithm.Log(
                            "[LONG UVXY]: The standard deviation of the 5-day rolling percent change of the VIX spot price is near zero, RSI is not extended in either direction and portfolio drawdown is below -12.5% [ Current UVXY RSI: {0} ]".format(
                                symbolData.RSI.Current.Value))


                # If the moving average cross over is between 4 and 5 standard deviations below its long term average value
                elif (symbolData.Mean - (5 * symbolData.STD) <= symbolData.EMACross.Current.Value <= symbolData.Mean - (
                        4 * symbolData.STD)
                      and Global.PortfolioDrawdown > -float(0.075)
                ):

                    HoldingPeriod = timedelta(hours=1.75)
                    insights.append(
                        Insight.Price(symbol, HoldingPeriod, InsightDirection.Up, float(0.01), None, None, 1.0))
    ...
    def ManagePosition(self, algorithm):
        ...
        for symbol, symbolData in self.LongVIX.items():
            ...
            if self.TradingWindow and algorithm.Portfolio[symbol].IsLong:

                # Insert comment here
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.045) and not algorithm.Portfolio[
                    SPXL].Invested and int(0) < VixHandler.FiveDayVixPercentMoveSTD < int(2):
                    RiskInsights.append(
                        Insight.Price(SPXL, timedelta(days=5), InsightDirection.Up, float(0.01), None, None, 0.5))
                    symbolData.Trade = False
...
class ShortVol(AlphaModel):
    ...
    def Update(self, algorithm, data):
        ...
        for symbol, symbolData in self.ShortVIX.items():
            ...
            if self.ReadyCheck(symbol, symbolData, algorithm):
                ...
                if (((symbolData.Mean - 1 * symbolData.STD) < symbolData.EMACross.Current.Value < (
                        symbolData.Mean - 0 * symbolData.STD))
                    ...
                    insights.append(
                        Insight.Price(symbol, timedelta(days=5), InsightDirection.Down, -float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log(
                        "[SHORT UVXY]: EMACross is negative but not extended, RSI is trending higher but not extended, UVXY gapped down, but less than 1 STD from the mean gap down, the VIX index isn't fluctuating wildly and there are currently no short UVXY positions")
                ...
                # If price is trending down, the current price is 3.5% below the average of the open and previous close, RSI is above half the average, and no other assets are being held
                      ...
                      and algorithm.Securities[symbol].Price < (
                              (Global.OpenClose[symbol][0] + Global.OpenClose[symbol][1]) / 2) * 0.975
                    ...
                    if Global.PortfolioDrawdown > -float(0.125):
                        ...
                        insights.append(
                            Insight.Price(symbol, RemainingTradingDay, InsightDirection.Down, -float(0.01), None, None,
                                          1.0))
                        symbolData.Trade = False
                        algorithm.Log(
                            "[SHORT UVXY]: EMACross is trending under the mean, RSI is above half of the long term mean RSI, the current price is 2.5% less than the mean of the previous close and recent open, the portfolio drawdown is greater than -12.5%, and UVXY is not currently a holding")

                    # Take a long-short position if the portfolio drawdown is greater than or equal to 12.5% because the market is likely experiencing very frequent high magnitude swings
                    else:

                        HoldingPeriod = max(datetime.combine(algorithm.Time.today(), time(15, 58)) - datetime.combine(
                            algorithm.Time.today(), algorithm.Time.time()), timedelta(minutes=10))

                        insights.append(
                            Insight.Price(symbol, HoldingPeriod, InsightDirection.Down, -float(0.01), None, None, 0.33))
                        insights.append(
                            Insight.Price(SQQQ, HoldingPeriod, InsightDirection.Up, float(0.01), None, None, 0.33))
                        insights.append(
                            Insight.Price(SPXL, HoldingPeriod, InsightDirection.Up, float(0.01), None, None, 0.33))

                        symbolData.Trade = False
                        algorithm.Log(
                            "[SHORT UVXY]: EMACross is trending under the mean, RSI is above half of the long term mean RSI, the current price is 2.5% less than the mean of the previous close and recent open, the portfolio drawdown is less than -12.5%, and UVXY is not currently a holding")
    ...
    def ManagePosition(self, algorithm, symbol, symbolData):
        ...
        if (
            ...
            if symbolData.EMACross.Current.Value > (symbolData.Mean + 0.75 * symbolData.STD):
                ...
                RiskInsights.append(
                    Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, float(0.01), None, None, 1.0))
                symbolData.Trade = False
                algorithm.Log("[SHORT UVXY] Switched to long UVXY due to extended crossover value")

            # If we are short and the price of UVXY increases by 3% or more in 1 minute we switch to long for at least 1 hour up to the rest of the trading day.
            elif symbolData.PriceWindow[0] / symbolData.PriceWindow[1] > 1.03 and time(9, 33) <= algorithm.Time.time():
                ...
                if datetime.combine(algorithm.Time.today(), time(16, 00)) - datetime.combine(algorithm.Time.today(),
                    ...
                    RemainingTradingDay = timedelta(minutes=60)
    ...
    def ReadyCheck(self, symbol, symbolData, algorithm):
        ...
        if (
                ...
                and symbolData.Trade
                and VixHandler.vixList
                and symbolData.PriceWindow.Count == 2
                and self.TradingWindow
