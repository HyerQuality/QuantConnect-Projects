            ...
            if self.TimeBounds[2] <= algorithm.Time.time() < self.TimeBounds[3]:
                ...
                if not data.ContainsKey(symbol):
                    ...
                    if symbolData.PriceWindow.IsReady:
                        ...
                        VixHandler.UVXYGap = Gap
                        symbolData.Gap = round(Gap,2)
                        
                        Weights.ShortUVXY  = round((1/(1.2446*VixHandler.PreviousVixClose.Current.Value - 0.1969*VixHandler.UVXYGap - 0.7731))*VixHandler.PreviousVixClose.Current.Value,4)
                        algorithm.Log("UVXY Gap Calculated at {1}: {0}".format(symbolData.Gap, algorithm.Time))
...
##-----------------Generate insights---------------------------------------------------##   
                ...
                if symbolData.EMACross.IsReady and symbolData.Trade:
                    ...
                    if VixHandler.vixList and self.TradingWindow: 
                        ...
                        # If UVXY gapped up more than 5 but less than 6 standard deviations from its average gap up
                        if symbolData.GapSignal[5] >= symbolData.Gap >= symbolData.GapSignal[4]: 
                            insights.append(Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, IntradayMagnitude))
                            symbolData.Trade = False 
                            algorithm.Log("UVXY gapped up more than 5 but less than 6 standard deviations from its average gap up")
                        
                        # If the previous day VIX % move was between +3 and +4 standard deviations from the mean
                        elif ( VixHandler.DeviationVixChange[6] <= VixHandler.vixPercentMoveList[4] < VixHandler.DeviationVixChange[7] ):  
                            
                            # And we are not currently invested and UVXY didn't gap up more than 10%  
                            if not algorithm.Portfolio[symbol].Invested and symbolData.GapSignal[0] >= symbolData.Gap > Zero:
                                insights.append(Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, IntradayMagnitude))
                                symbolData.Trade = False
                                algorithm.Log("The previous day VIX % move was between +3 and +4 standard deviations from the mean and we are not currently invested and UVXY didn't gap up more than 10%")

                        # If UVXY gapped down more than 4 standard deviations from its average gap down
                        elif symbolData.Gap <= symbolData.GapDownSignal[3]: 
                            insights.append(Insight.Price(symbol, RemainingTradingDay, InsightDirection.Up, IntradayMagnitude))
                            symbolData.Trade = False
                            algorithm.Log("UVXY gapped down more than 4 standard deviations from its average gap down")
                        ...
                        elif int(0) < VixHandler.FiveDayVixPercentMoveSTD < int(2) and not algorithm.Portfolio[symbol].Invested and int(70) >= symbolData.RSI.Current.Value >= int(20):
                            insights.append(Insight.Price(symbol, timedelta(hours=10), InsightDirection.Up, IntradayMagnitude))
                            symbolData.Trade = False
                            algorithm.Log("The standard deviation of the 5-day rolling percent change of the VIX spot price is near zero and RSI is not extended in either direction [ Current UVXY RSI: {0} ]".format(symbolData.RSI.Current.Value))
                        ...
                        # If the moving average cross over is between 4 and 5 standard deviations below its long term average value
                            ...
                            insights.append(Insight.Price(symbol, timedelta(hours=1.25), InsightDirection.Up, float(0.01)))
