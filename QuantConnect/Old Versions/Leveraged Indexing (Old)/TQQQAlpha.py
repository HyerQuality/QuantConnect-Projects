            ...
            if self.TimeBounds[2] <= algorithm.Time.time() < self.TimeBounds[3]:
                ...
                if not data.ContainsKey(symbol):
                    ...
                    if symbolData.PriceWindow.IsReady:
                        ...
                        algorithm.Log("TQQQ Gap Calculated at {1}: {0}".format(symbolData.Gap, algorithm.Time))
                    else:
                        Gap = Zero
...
##-----------------Generate insights---------------------------------------------------##   
                ...
                if symbolData.EMACross.IsReady and symbolData.Trade and VixHandler.vixList:
                    ...
                    if symbolData.RSIDeviations[0] <= symbolData.RSI.Current.Value <= symbolData.RSIDeviations[1]:
                        ...
                        algorithm.Log("The RSI value is less than or two standard deviations from the mean [ Current TQQQ RSI: {0}".format(symbolData.RSI.Current.Value))
                    ...
                    # If we are not currently invested in TQQQ, the EMA Cross is between -1.5 and -1 standard deviations from the mean, and the 5 day standard deviation of VIX close is greater than 7
                        ...
                        insights.append(Insight.Price(symbol, timedelta(days=1), InsightDirection.Up, IntradayMagnitude))
                        symbolData.Trade = False
                        algorithm.Log("We are not currently invested in TQQQ, the EMA Cross is between -1.5 and -1 standard deviations from the mean, and the 5 day standard deviation of VIX close is greater than 7")
        ...
        for symbol, symbolData in self.TQQQ.items():
            if self.TradingWindow and not algorithm.Portfolio[SQQQ].Invested:
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.05):
                    RiskInsights.append(Insight.Price(SQQQ, timedelta(hours=0.5), InsightDirection.Up, IntradayMagnitude))
    ...
    def OnSecuritiesChanged(self, algorithm, changes):
        ...
        for added in changes.AddedSecurities:
            ...
            if symbolData is None and added.Symbol == TQQQ:
                # Create indicators
                symbolData = TQQQSymbolData(added)
