        ...
        for symbol, symbolData in self.TQQQ.items():
            ...
            if self.ReadyCheck(symbol, symbolData):
                ...
                # If we are not currently invested in TQQQ, the EMA Cross is between -1.5 and -1 standard deviations from the mean, and the 5 day standard deviation of VIX close is greater than 7
                    ...
                    insights.append(Insight.Price(symbol, timedelta(days=1), InsightDirection.Up, float(0.01), None, None, 1.0))
                    symbolData.Trade = False
                    algorithm.Log("[TQQQ]: We are not currently invested in TQQQ, the EMA Cross is between -1.5 and -1 standard deviations from the mean, and the 5 day standard deviation of VIX close is greater than 7")
        ...
        for symbol, symbolData in self.TQQQ.items():
            if self.TradingWindow and not algorithm.Portfolio[SQQQ].Invested:
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.05):
                    RiskInsights.append(Insight.Price(SQQQ, timedelta(hours=0.5), InsightDirection.Up, float(0.01), None, None, 0.5))
    ...
    def ManagePosition(self, algorithm):
        ...
        for symbol, symbolData in self.SQQQ.items():
            if self.TradingWindow and algorithm.Portfolio[symbol].Invested:
                if algorithm.Portfolio[symbol].UnrealizedProfitPercent <= -float(0.05):
                    RiskInsights.append(Insight.Price(TQQQ, timedelta(hours=1.0), InsightDirection.Up, float(0.01), None, None, 0.5))
