...
class ThetaArb(QCAlgorithm):
    '''Day trades TVIX.  Buy and hold leveraged SPY.  Fluctuate risk using volatitly ETNs. Tesla discriminately used as a Beta hedge. Relies on the assumption Tesla is successful'''
    ...
    def Initialize(self):
        ...
        self.SetCash(57451.50)                                       #Set Strategy Cash
        ...
        self.MaxProportion = 0.32  #Set to a different value than intended to check that VIX data is loaded and variable is being adjusted
...
#OnData 
    def OnData(self, data):
        ...
        self.MaxProportion = round(np.minimum(float(100/((37/self.ClosingVix) +1))/100, 0.245),3)       
...
#MarginCall
    def MarginCall(self):
        ...
        #The portfolio has received a margin call. Rebalance to free up capital. Margin calls are OhShit moments
        if self.Securities["TVIX"] not in self.OpenOrders:
            self.SetHoldings("TVIX", -self.MaxProportion)    
        if self.Securities["SPXL"] not in self.OpenOrders:
            self.SetHoldings("SPXL", 0.13)  
...
#TVIX
    def TvixFunc(self):
        ...
        elif ( (1.57 > self.EMADiff.Current.Value >= 1.37) and self.vixRsi.Current.Value > 40 ):
            if self.Securities["TVIX"] not in self.OpenOrders:
        ...
        #If the indicator value is within this range position is hedged and buying the SPY dip.
        elif ( (1.37 > self.EMADiff.Current.Value >= 0.972) and self.vixRsi.Current.Value > 40 ):
            if self.Securities["TVIX"] not in self.OpenOrders:
